(function () {
   ...
   */ const p = Symbol("Comlink.proxy"),
    _ = Symbol("Comlink.endpoint"),
    j = Symbol("Comlink.releaseProxy"),
    m = Symbol("Comlink.finalizer"),
    C = Symbol("Comlink.thrown"),
    x = (u) => (typeof u == "object" && u !== null) || typeof u == "function",
    V = {
      canHandle: (u) => x(u) && u[p],
      serialize(u) {
      ...
      serialize({ value: u }) {
        ...
        return (
          u instanceof Error
            ? (t = {
                isError: !0,
                value: { message: u.message, name: u.name, stack: u.stack },
              })
            : (t = { isError: !1, value: u }),
          [t, []]
        );
  ...
  function U(u, t) {
    for (const e of u)
      if (t === e || e === "*" || (e instanceof RegExp && e.test(t))) return !0;
    return !1;
  }
  function h(u, t = globalThis, e = ["*"]) {
    t.addEventListener("message", function a(r) {
      ...
      const { id: c, type: l, path: n } = Object.assign({ path: [] }, r.data),
        E = (r.data.argumentList || []).map(A);
      let i;
      try {
        const s = n.slice(0, -1).reduce((F, d) => F[d], u),
          o = n.reduce((F, d) => F[d], u);
        switch (l) {
          case "GET":
            i = o;
            break;
          case "SET":
            (s[n.slice(-1)[0]] = A(r.data.value)), (i = !0);
            break;
          case "APPLY":
            i = o.apply(s, E);
            break;
      ...
      Promise.resolve(i)
        ...
        .then((s) => {
          const [o, F] = y(s);
          t.postMessage(Object.assign(Object.assign({}, o), { id: c }), F),
            l === "RELEASE" &&
              (t.removeEventListener("message", a),
              O(t),
              m in u && typeof u[m] == "function" && u[m]());
        })
        .catch((s) => {
          const [o, F] = y({
  ...
  function P(u) {
    return g(u, new Map(), { type: "RELEASE" }).then(() => {
      O(u);
    });
  }
  const f = new WeakMap(),
  ...
  function W(u, t) {
    const e = (f.get(t) || 0) + 1;
    f.set(t, e), B && B.register(u, t, u);
  }
  function Y(u) {
    B && B.unregister(u);
  }
  function b(u, t, e = [], a = function () {}) {
    let r = !1;
    const c = new Proxy(a, {
      get(l, n) {
        if ((D(r), n === j))
      ...
      set(l, n, E) {
        ...
        return g(
          u,
          t,
          { type: "SET", path: [...e, n].map((o) => o.toString()), value: i },
          s,
        ).then(A);
      },
      apply(l, n, E) {
        ...
        if (i === _) return g(u, t, { type: "ENDPOINT" }).then(A);
        if (i === "bind") return b(u, t, e.slice(0, -1));
        const [s, o] = z(E);
        return g(
          u,
          t,
          { type: "APPLY", path: e.map((F) => F.toString()), argumentList: s },
          o,
        ).then(A);
      },
      construct(l, n) {
  ...
  function Z(u) {
    return Object.assign(u, { [p]: !0 });
  }
  function y(u) {
    for (const [t, e] of M)
      if (e.canHandle(u)) {
        const [a, r] = e.serialize(u);
        return [{ type: "HANDLER", name: t, value: a }, r];
      }
    return [{ type: "RAW", value: u }, T.get(u) || []];
  }
  function A(u) {
    switch (u.type) {
      case "HANDLER":
        return M.get(u.name).deserialize(u.value);
      case "RAW":
        return u.value;
    }
  }
  function g(u, t, e, a) {
    return new Promise((r) => {
      ...
      t.set(c, r),
        u.start && u.start(),
        u.postMessage(Object.assign({ id: c }, e), a);
    });
  }
  function q() {
    return new Array(4)
  ...
  function X(u) {
    if (typeof u != "string")
      throw new TypeError(
        `Expected 'input' to be of type string, but received '${u}'`,
      );
    return u.length
      ? u.replace(/(\S)/g, (t, e) => {
          const a = K.find((r) => new RegExp(r.diacritics).test(e));
          return a == null ? e : a.letter;
        })
      : u;
  }
  let k = null,
    L = null;
  const w = (u) => X(u.replace("#", "").toLowerCase()),
    S = (u) => ({
      id: u.id,
      label: u.label.replace("#", ""),
      normalizedLabel: w(u.label),
      boundingBox: u.boundingBox,
    }),
    R = (u) => [
      ...u.layer1.children.map(({ path: t }) => S(t)),
      ...u.layer2.children.map(({ path: t }) => S(t)),
      ...u.layer3.groups.flatMap((t) =>
        t.children.map(({ rect: e }) => ({
          id: e.id,
          label: e.label.replace("#", ""),
          normalizedLabel: w(e.label),
        ...
        [...u.values()].forEach(
          ({
            ...
            a.forEach((E) => {
              ...
              if (!e.has(E)) {
                const o =
                  ((i = t.get(E)) == null ? void 0 : i.key) ?? "UNKNOWN";
                e.set(E, {
                  id: E,
                  articlesCount: n,
                  name: o,
                  nameNormalized: o.toLowerCase(),
                  clusters: [],
                });
              }
              (s = e.get(E)) == null ||
                s.clusters.push({ clusterId: r, x: c, y: l });
            });
          },
        ),
        e
  ...
  var $ = Object.freeze({
    __proto__: null,
    createClustersByConcept: N,
    createLabelsCollection: R,
    search: (u, t) => {
      const e = k ?? (k = R(u.map)),
        a = L ?? (L = N(u.dataPoints, u.concepts));
      if (!t) return { labels: [], points: [] };
      const r = w(t).toLowerCase(),
        c = () =>
          [...a.values()]
            .filter(({ nameNormalized: E }) => E.includes(r))
            .sort((E, i) => i.clusters.length - E.clusters.length)
            .slice(0, 300);
      return {
        labels: e.filter(({ normalizedLabel: l }) => l.includes(r)),
        points: c(),
      };
    },
  });
